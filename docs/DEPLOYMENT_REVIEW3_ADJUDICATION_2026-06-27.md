...
> ⚠️ **后续调整（按需求简化）**：R31 大小模型智能路由 / R32 小模型 systemd 已**撤销**——
> 全平台改为**单一模型**，且 AIOps 诊断与 AI 助手**统一**从 `model_agents`/"模型服务"（→ `llm.yaml`/env 回退）取模型。
> 不再有 `SMALL_LLM_*` 环境变量、`_llm_endpoints` 路由与 `autops-llm-small.service`。R27 的可配置超时保留。
