<template>
  <div class="page-container">
    ...
    <el-table stripe :data="filteredKeys" v-loading="loading"border>
      ...
      <el-table-column label="使用" width="130" align="center">
        <template #default="{ row }">
          <div class="usage-cell">
            ...
            <span style="color:#86909c;font-size:11px">次</span>
          </div>
        </template>
      </el-table-column>
    ...
    <!-- 创建对话框 -->
    <el-dialog v-model="dialogVisible" title="创建 API Key" width="600px">
    ...
    <!-- 密钥展示对话框 -->
    <el-dialog v-model="secretDialogVisible" title="API Key 创建成功" width="600px" :close-on-click-modal="false">
      ...
      <template v-if="detailData">
        ...
        <div class="usage-chart">
          <div v-for="(bar, i) in usageBars" :key="i" class="usage-bar-item">
            <div class="usage-bar" :style="{height: bar.height + '%', background: '#165dff'}"></div>
            <span class="usage-bar-label">{{ bar.label }}</span>
          </div>
        </div>

        <h4 style="margin:16px 0 8px">使用审计 (最近20条)</h4>
        <el-table stripe :data="auditLogs"size="small" v-loading="auditLoading">
...
function expiryColor(row: any) {
  if (isExpired(row)) return '#f53f3f'
  if (isExpiringSoon(row)) return '#ff7d00'
  return '#00b42a'
}
