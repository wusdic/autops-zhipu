          ...
          <div class="autops-card result-card" :class="simulateResult.matched ? 'matched' : 'unmatched'">
            <div class="result-header">
              ...
              <div>
                ...
                <p style="margin:4px 0 0;color:#909399">{{ simulateResult.matched ? '模拟条件满足策略触发条件' : '当前条件不满足任何触发条件' }}</p>
              </div>
            </div>
          ...
          <div class="autops-card" v-if="simulateResult.condition_details?.length" style="margin-top:16px">
            ...
            <el-table :data="simulateResult.condition_details" stripe>
              ...
              <el-table-column label="运算符" width="80" align="center">
                <template #default="{ row }">
                  <span style="font-weight:bold;color:#409eff">{{ row.operator }}</span>
                </template>
              </el-table-column>
              ...
              <el-table-column label="匹配" width="80" align="center">
                <template #default="{ row }">
                  <el-icon :size="18" :color="row.matched ? '#67c23a' : '#f56c6c'">
            ...
            <div v-if="simulateResult.condition_logic" style="margin-top:8px;color:#909399;font-size:13px">
          ...
          <div class="autops-card" v-if="simulateResult.actions?.length" style="margin-top:16px">
            ...
            <el-timeline>
              <el-timeline-item v-for="(act, idx) in simulateResult.actions" :key="idx"
                ...
                <div class="action-preview-card">
                  ...
                  <div v-if="act.description" style="color:#909399;font-size:13px;margin-top:4px">{{ act.description }}</div>
            ...
            <div class="flow-chain">
              <div v-for="(act, idx) in simulateResult.actions" :key="idx" class="flow-step">
                <div class="flow-step-box" :style="{borderColor: getActionColor(act.type) === 'danger' ? '#f56c6c' : getActionColor(act.type) === 'warning' ? '#e6a23c' : '#409eff'}">
...
.asset-info-box { margin: -8px 0 12px 89px; padding: 10px; background: #f5f7fa; border-radius: 6px; border: 1px solid #ebeef5; }
.metric-inputs { background: #f5f7fa; padding: 10px; border-radius: 4px; }
...
.result-card.matched { border-left: 4px solid #67c23a; }
.result-card.unmatched { border-left: 4px solid #909399; }
.result-header { display: flex; gap: 16px; align-items: center; }
.mini-stat { text-align: center; padding: 8px; background: #f5f7fa; border-radius: 6px; }
.mini-value { font-size: 16px; font-weight: bold; color: #303133; }
...
.action-preview-card { background: #fafbfc; padding: 10px; border-radius: 6px; border: 1px solid #ebeef5; }
...
.action-params code { font-size: 12px; color: #606266; white-space: pre-wrap; }
.action-meta { margin-top: 6px; display: flex; gap: 12px; color: #909399; font-size: 12px; }
...
.flow-step-box { border: 2px solid #409eff; border-radius: 8px; padding: 12px 20px; text-align: center; min-width: 180px; background: #fff; }
.flow-step-num { font-size: 18px; font-weight: bold; color: #409eff; }
.flow-step-name { font-size: 13px; font-weight: 600; margin-top: 2px; }
.flow-step-target { font-size: 11px; color: #909399; margin-top: 2px; }
