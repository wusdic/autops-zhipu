<template>
  <div class="autops-page-container">
    ...
    <!-- 创建/编辑对话框 -->
    <el-dialog v-model="showDialog" :title="editing?'编辑规则':'新建规则'" width="780px" top="5vh">
      ...
      <template v-if="detail">
        ...
        <div v-if="detail.conditions?.length">
          <div v-for="(c,i) in detail.conditions" :key="i" style="margin-bottom:6px">
            ...
            <span v-if="c.duration" style="color:#165dff;margin-left:4px">持续 {{ c.duration }}</span>
          </div>
          <p style="color:#86909c;font-size:13px;margin-top:4px">逻辑: {{ detail.condition_logic || 'AND' }}</p>
        </div>

        <h4 style="margin:16px 0 8px">近期触发记录</h4>
        <el-table stripe :data="triggerHistory"size="small">
...
.stat-card--info .stat-value { color: var(--autops-info); } 
