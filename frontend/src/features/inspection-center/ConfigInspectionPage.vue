<template>
  <div class="page-container">
    ...
    <div class="autops-card">
      <el-table :data="filteredItems" v-loading="loading" stripe class="autops-table" @sort-change="handleSortChange">
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link type="warning" @click="runSingle(row)" :loading="row._running">执行</el-button>
            <el-button link type="danger" v-if="row.status === 'abnormal'" @click="createAnomaly(row)">报异常</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 详情对话框 -->
    <el-dialog v-model="detailVisible" title="配置巡检详情" width="700px" destroy-on-close>
      ...
      <div v-if="currentItem?.diff_content" style="margin-top: 16px">
        ...
        <div class="diff-content">
          <pre style="background: #1e1e1e; color: #d4d4d4; padding: 12px; border-radius: 4px; font-size: 12px; max-height: 300px; overflow: auto">{{ currentItem.diff_content }}</pre>
        </div>
      </div>
