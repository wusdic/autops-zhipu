...
async def on_alert_created_match_policy(event: DomainEvent) -> None:
    """告警创建时匹配策略：统一由 PolicyService 落 PolicyExecution 再发事件."""
    ...
    try:
        ...
        if not severity and not event_type:
            logger.debug("policy: 告警事件缺少 severity/event_type, 跳过策略匹配")
            return
        ...
        async with async_session_factory() as session:
            ...
            plans = await svc.match_and_record(
        ...
        if plans:
            ...
            for plan in plans:
                await bus.publish(DomainEvent(
                    ...
                    domain="policy",
                    payload={**plan, "context": context},
                    source="policy_engine",
                    correlation_id=event.correlation_id or event.event_id,
                ))
            logger.info("policy: 告警匹配并落库 %d 条策略执行 alert_id=%s", len(plans), alert_id)
        else:
