...
async def dashboard_stats(db: AsyncSession = Depends(get_db)):
    ...
    # 告警统计
    alert_open = (await db.execute(
        select(func.count()).select_from(Alert).where(Alert.status == "open")
    )).scalar() or 0

    # 异常统计
    anomaly_open = (await db.execute(
        select(func.count()).select_from(Anomaly).where(Anomaly.status.in_(["new", "confirmed", "analyzing"]))
    )).scalar() or 0

    # 执行统计
    exec_pending = (await db.execute(
        select(func.count()).select_from(Execution).where(Execution.status == "pending_approval")
    )).scalar() or 0
...
async def inspection_summary(db: AsyncSession = Depends(get_db)):
    """巡检概况."""
    try:
        from app.domains.inspection.models import InspectionTask, InspectionResult
        task_total = (await db.execute(
            select(func.count()).select_from(InspectionTask)
        )).scalar() or 0
        ...
        # 异常项统计
        abnormal_items = (await db.execute(
            select(func.count()).select_from(InspectionResult)
            .where(InspectionResult.status == "abnormal")
        )).scalar() if hasattr(InspectionResult, 'status') else 0

        success_rate = round(task_completed / max(task_total, 1) * 100, 1)
    except Exception:
        task_total, task_completed, task_failed, abnormal_items, success_rate = 0, 0, 0, 0, 0
...
async def anomaly_summary(db: AsyncSession = Depends(get_db)):
    ...
    status_rows = (await db.execute(
        select(Anomaly.status, func.count().label("count"))
...
async def report_summary(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        total = (await db.execute(
            select(func.count()).select_from(ReportTask)
        )).scalar() or 0
        completed = (await db.execute(
            select(func.count()).select_from(ReportTask)
...
async def platform_health_summary(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        checks["redis"] = {"status": "unhealthy", "message": str(e)[:100]}
...
async def my_pending_items(
    ...
    pending_anomalies = (await db.execute(
        select(func.count()).select_from(Anomaly)
        .where(Anomaly.assigned_to == user_id, Anomaly.status.in_(["new", "confirmed"]))
    )).scalar() or 0

    # 待处理工单
    pending_tickets = (await db.execute(
        select(func.count()).select_from(Ticket)
        .where(Ticket.assigned_to == user_id, Ticket.status.in_(["open", "in_progress"]))
    )).scalar() or 0
